...
@Named
@ViewScoped
public class LawReasonAction extends BaseAction<LawReason>{
    ...
    @Override
    public List<LawReason> getList() {
        if(super.getList()==null || super.getList().isEmpty()){
            Map<String,Object> params = new HashMap<String,Object>();
            params.put("mrchntid",merchant.getId());
           super.setList(new ArrayList<LawReason>());
           if(Helper.getCurrentUserIsAdmin()){
               super.getList().addAll(getCrud().getNamedList("LawReason.findAll"));
           }else{
                super.getList().addAll(getCrud().getNamedList("LawReason.findLawReasonByMerchant",params));
           }
        }
