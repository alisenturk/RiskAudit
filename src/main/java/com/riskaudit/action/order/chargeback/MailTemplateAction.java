...
@Named
@ViewScoped
public class MailTemplateAction extends BaseAction<MailTemplate>{
    ...
    public List<MailTemplate> getList() {
        if(super.getList()==null || super.getList().isEmpty()){
           ...
           super.setList(new ArrayList<MailTemplate>());
           if(Helper.getCurrentUserIsAdmin()){
               super.getList().addAll(getCrud().getNamedList("MailTemplate.findAll"));
           }else{
                super.getList().addAll(getCrud().getNamedList("MailTemplate.findMailTemplateByMerchant",params));
           }
        }
