...
@Named
@ViewScoped
public class DocumentTypeAction  extends BaseAction<DocumentType>{
    ...
    public List<DocumentType> getList() {
        if(super.getList()==null || super.getList().isEmpty()){
           ...
           super.setList(new ArrayList<DocumentType>());
           if(Helper.getCurrentUserIsAdmin()){
               super.getList().addAll(getCrud().getNamedList("DocumentType.findAll"));
           }else{
                super.getList().addAll(getCrud().getNamedList("DocumentType.findDocumentTypeByMerchant",params));
           }
        }
